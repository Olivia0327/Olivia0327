...
function  updateGtid(gtid,$select){
	...
	//在业务层接口扩展一个更新商品类别的方法
	//在业务层实现类实现相应的方法
	//在数据层DaO接口扩展一个更新商品类别的方法
	//在dao对就的mapper文件中相应的update语句
	//在客户端处理服务端返回的结果，显示消息气泡
}

$.ajax({
	url : "rest/hongyan/goods/update/type",//最前面不要斜杠
	type : "post",
...
function baocun(a){
	...
	var $input1 = $tr.children("td:eq(2)").children("input");//获取第一个文本框
	...
	var data = { "goodsid": goodsid,     "name" : $input1.val(), 
			     "spec" : $input2.val(), "price" : $input3.val(),
			     "amount" : $input4.val(),};
	
	$.ajax({
		url : "GoodsUpdate",//最前面不要斜杠
		type : "post",
		data : data,
		success : function(r){
...
function xiugai(a){
	...
	var $tr = $tr.children("td:eq(2)");
	var $tr = $tr.children("td:eq(3)");
	var $tr = $tr.children("td:eq(4)");
	var $tr = $tr.children("td:eq(5)");//取得第六个单元格
	
	editTd($td3);//调整函数修改单元格变成文本框
	editTd($td4);
	editTd($td5);
	editTd($td6);
